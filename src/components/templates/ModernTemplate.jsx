...
export default function ModernTemplate({ data, sectionOrder, colorTheme }) {
  ...
  const sections = {
    ...
    experience: filledExperience.length > 0 && (
      <div key="experience" className="mb-6">
        <h2 className="text-[13px] font-bold uppercase tracking-wider mb-3 flex items-center gap-2" style={{ color: primary }}>
          <div className="w-6 h-0.5 rounded" style={{ backgroundColor: primary }}></div>
          Experience
        </h2>
        <div className="space-y-4 pl-8">
          {filledExperience.map((exp) => (
            <div key={exp.id} className="relative">
              <div className="absolute -left-5 top-1.5 w-2 h-2 rounded-full" style={{ backgroundColor: primary }}></div>
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-bold text-[12px] text-gray-900">{exp.position}</h3>
                  <p className="font-medium text-[11px]" style={{ color: primary }}>{exp.company}</p>
                </div>
          ...
          {filledEducation.map((edu) => (
            <div key={edu.id} className="flex justify-between items-start">
              <div>
                ...
                <p className="text-[11px]" style={{ color: primary }}>{edu.institution}</p>
      ...
      <div key="skills" className="mb-6">
        ...
        <div className="flex flex-wrap gap-2 pl-8">
          {filledSkills.map((skill, index) => (
            <span key={index} className="px-3 py-1 rounded-full text-[10px] font-medium border" style={{ backgroundColor: primaryLight, color: primary, borderColor: primary + '30' }}>
              {skill}
            </span>
          ))}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="text-white px-10 py-8" style={{ background: `linear-gradient(135deg, ${primary}, ${primary}dd)` }}>
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            {personalInfo.title && (
              <p className="text-[14px] font-medium tracking-wider uppercase mb-4 truncate" style={{ color: 'rgba(255,255,255,0.8)' }}>
                {personalInfo.title}
              </p>
            )}
            <div className="flex flex-wrap gap-x-5 gap-y-1 text-[10px]" style={{ color: 'rgba(255,255,255,0.8)' }}>
              {personalInfo.email && (
