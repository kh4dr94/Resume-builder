...
export default function ClassicTemplate({ data, sectionOrder, colorTheme }) {
  ...
  const accentColor = accent || '#1f2937' // gray-800
  const borderColor = accent || '#9ca3af' // gray-400
    ...
    summary: () =>
      personalInfo.summary ? (
        <div key="summary" className="mb-5">
          <h2 className="text-[12px] font-bold uppercase tracking-[2px] pb-1 mb-2" style={{ color: accentColor, borderBottom: `1px solid ${borderColor}` }}>
    ...
    experience: () =>
      filledExperience.length > 0 ? (
        <div key="experience" className="mb-5">
          <h2 className="text-[12px] font-bold uppercase tracking-[2px] pb-1 mb-3" style={{ color: accentColor, borderBottom: `1px solid ${borderColor}` }}>
          ...
          <div className="grid grid-cols-3 gap-x-4 gap-y-1 mt-2">
            {filledSkills.map((skill, index) => (
              <span key={index} className="text-gray-700 text-[10px] flex items-center gap-1.5">
                <span className="w-1 h-1 rounded-full" style={{ backgroundColor: accent || '#9ca3af' }}></span>
                {skill}
              </span>
            ))}
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-6 pb-5" style={{ borderBottom: `2px solid ${accent || '#111827'}` }}>
        <h1 className="text-[30px] font-normal tracking-[4px] uppercase mb-1" style={{ fontFamily: "'Georgia', serif", color: accent || '#111827' }}>
